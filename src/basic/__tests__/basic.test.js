...
describe('basic test', () => {
  describe.each([{ type: 'basic', loadFile: () => import('../main.basic.js') }])(
    '$type 장바구니 시나리오 테스트',
    ({ loadFile }) => {
      ...
      it('총액이 올바르게 계산되는지 확인', () => {
        ...
        const expected = checkDiscountDay()
          ? '총액: 18000원(10.0% 할인 적용)(포인트: 18)'
          : '총액: 20000원(포인트: 20)';

        expect(sum.textContent).toContain(expected);
      });
      ...
      it('할인이 올바르게 적용되는지 확인', () => {
        ...
        const expected = checkDiscountDay() ? '(10.0% 할인 적용)' : '';
        expect(sum.textContent).toContain(expected);
      });
      ...
      it('포인트가 올바르게 계산되는지 확인', () => {
        sel.value = 'p2';
        addBtn.click();
        const expected = checkDiscountDay() ? '(포인트: 115)' : '(포인트: 128)';
        expect(document.getElementById('loyalty-points').textContent).toContain(expected);
      });
